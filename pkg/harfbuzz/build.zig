...
pub fn build(b: *std.Build) !void {
    ...
    if (b.systemIntegrationOption("harfbuzz", .{})) {
        // A separate root keeps test links out of the exported module.
        const test_module = b.createModule(.{
            .root_source_file = b.path("main.zig"),
            .target = target,
            .optimize = optimize,
        });
        test_module.import_table = module.import_table;
        test_module.linkSystemLibrary("harfbuzz", dynamic_link_opts);
        if (freetype_enabled and b.systemIntegrationOption("freetype", .{}))
            test_module.linkSystemLibrary("freetype2", dynamic_link_opts);
        test_exe.root_module = test_module;
    }
...
fn buildLib(b: *std.Build, options: anytype) !*std.Build.Step.Compile {
    ...
    if (freetype_enabled) {
        ...
        if (b.systemIntegrationOption("freetype", .{})) {
            // Static consumers link system libraries at the final executable.
            if (freetype.builder.lazyDependency("freetype", .{})) |ft_upstream| {
                lib.root_module.addIncludePath(ft_upstream.path("include"));
            }
        } else {
...
const HarfBuzzC = struct {
    ...
    fn addImportToModule(
        ...
        // Keep pkg-config inputs on the translator, but do not embed shared
        // libraries in static consumers of this generated bindings module.
        hb_c.mod.link_objects.clearRetainingCapacity();
