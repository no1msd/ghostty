...
const PosixPty = struct {
    ...
    // Zig 0.16 mistranslates glibc's fortified inline ptsname_r wrapper
    // (a C integer expression becomes bool). Call the libc ABI directly;
    // the explicit buffer length still bounds the write.
        ...
        return switch (info) {
            ...
            .tty_name => {
                ...
                switch (builtin.os.tag) {
                    ...
                    .linux => {
                        if (ptsname_r(self.master, &self.tty_name_buf, self.tty_name_buf.len) != 0) return null;
                        const tty_name: [:0]const u8 = std.mem.sliceTo(&self.tty_name_buf, 0);
                        self.tty_name = tty_name;
                        return tty_name;
                    },
